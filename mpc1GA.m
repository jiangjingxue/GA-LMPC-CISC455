...
generation_limit = 20;           % iteration limit of the genetic algorithm
num_children = 2;                % number of children produced every iteration (no more than 2)
population_size = 20;            % population size
tournament_size = 10;             % 0.2 * population size 
...
[population,queue] = initialize_population_heuristic_one(population_size); 
% [population,queue] = initialize_population_power(population_size);
fitness = [];
...
while(IsTerminationSatisfied(termination_counter,generation_limit) ~= 1)
    ...
    % parents_index = sus(population_size, fitness, pool_size);


    % implement crossover with probability
    r = rand;
    child_fitness_value = 0; 
    ...
    if r < crossover_rate
        ...
        % [child, child_fitness_value] = disruptive_crossover(population(parents_index(1)),population(parents_index(2)),xActual,u_k,du,total_sim_iter,v_k,theta_k,dT,sys,i_ref,D,c,refTrajectory,unicycle,x_ref,y_ref,theta_ref,v_ref,w_ref);
    ...
    % [population,fitness,queue] = fitness_based_replacement(population,fitness,queue,children,children_fitness);
    % [population,fitness,queue] = all_children(population,fitness,queue,children,children_fitness);
...
[~,xActual,D,u_k,du,v_k,theta_k,cte,x_error,y_error,points,euclid_error,fval] = evaluate_fitness(xActual,u_k,du,total_sim_iter,v_k,theta_k,dT,sys,i_ref,D,c,refTrajectory,unicycle,x_ref,y_ref,theta_ref,v_ref,w_ref,population(best_index));
